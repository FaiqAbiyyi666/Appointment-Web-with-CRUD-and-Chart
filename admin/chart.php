...
$connect = mysqli_connect("localhost", "root", "", "sql_database_edoc");
if ($connect) {
    // Retrieve scheduled appointments from the database
    $query = "SELECT DATE_FORMAT(s.scheduledate, '%M %Y') AS month_year, COUNT(*) AS count
              FROM schedule s
              JOIN appointment a ON s.scheduleid = a.scheduleid
              GROUP BY month_year
              ORDER BY STR_TO_DATE(CONCAT('01 ', month_year), '%d %M %Y')";
    $result = mysqli_query($connect, $query);

    // Initialize an array to store scheduled appointments
    $appointment = array();

    // Loop through the data and populate the array
    while ($row = mysqli_fetch_assoc($result)) {
        $monthYear = $row['month_year'];
        $appointment[$monthYear] = $row['count'];
    }
} else {
    die("Koneksi ke database gagal: " . mysqli_connect_error());
}

$labels = array_keys($appointment);
$data = array_values($appointment);
?>
...
<body>
    ...
    <tr class="menu-row">
        <td class="menu-btn menu-icon-chart menu-active menu-icon-chart-active">
            <a href="chart.php" class="non-style-link-menu non-style-link-menu-active">
                <div>
                    <p class="menu-text">Chart</p>
            </a></div></a>
        </td>
    </tr>
    </table>
    ...
    <div class="dash-body">
        <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
            <tr>
                <td width="13%">
                    <a href="chart.php"><button class="login-btn btn-primary-soft btn btn-icon-back" style="padding-top:11px;padding-bottom:11px;margin-left:20px;width:125px">
                            <font class="tn-in-text">Back</font>
                        </button></a>
                </td>
                <td>
                    <p style="font-size: 23px;padding-left:12px;font-weight: 600;">Appointment Chart</p>
        ...
        <script>
            ...
            const appointmentData = {
                labels: labels,
                datasets: [{
                    label: 'Laporan Appointment Per-Bulan',
            ...
            var appointmentChart = new Chart(
        ...
        if ($action == 'add-session') {
            ...
            <div id="popup1" class="overlay">
                    ...
                    <center>
                        ...
                        <a class="close" href="chart.php">&times;</a> 
